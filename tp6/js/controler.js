    ...
    function ($scope, $http) {
        ...
        $scope.GPS = function () {
            navigator.geolocation.getCurrentPosition(function (position) {
                /* appel AJAX à l’API openweathermap */
                $http.get('https://api.openweathermap.org/data/2.5/weather?lat=' + position.coords.latitude + '&lon=' + position.coords.longitude + '&units=metric&appid=ae3e8be45d47ccd5941b17ff8f3aba94')
                    .success(function (data) {
                        /* on met dans l’objet ​meteo les données retournées
                         par openweathermap */
                        $scope.meteo = data;
